...
public class KOTHCommandHandler implements CommandExecutor {
    ...
    private Map<Command, KOTHCommand> commands = new HashMap<>();

    public KOTHCommandHandler(Plugin plugin) {
        for(Class<?> clazz : Avalon.get().getClassesInPackage("me.aventium.avalon.koth.command.commands")) {
            try {
                if(clazz.getAnnotation(Command.class) != null) {
                    Command c = clazz.getAnnotation(Command.class);
                    KOTHCommand command = (KOTHCommand) clazz.newInstance();
    ...
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String label, String[] args) {
        ...
        if(args.length == 0) {
            player.sendMessage("§7----[ §6KOTH Commands §7]----");
        ...
        if(cmd == null) {
            player.sendMessage("§7Unknown KOTH command! Use /§6" + label + " §7for help on managing KOTH!");
            return true;
        }
